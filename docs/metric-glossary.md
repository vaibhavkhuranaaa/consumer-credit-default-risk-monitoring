...
| Minimum simulated capacity | Research-score rank divided by the 30,000-row denominator | Analyst artifact | Smallest artifact share whose deterministic rank cutoff reaches the record | Not a policy threshold, operational queue assignment, or lending decision |
| Selected-scenario cutoff | Floor of the selected 5%, 10%, 20%, 35%, or 50% capacity multiplied by the artifact denominator | Selected capacity plus denominator | Places the record's fixed rank in context for one retrospective workload scenario | Not what a lender decided; never approval, denial, eligibility, pricing, or recommendation |
...
The record-level interface shows only academic source ID, retrospective score, band, rank/denominator, minimum simulated capacity, selected-scenario cutoff context, and historical outcome. Lending-decision, eligibility, pricing, adverse-action, recommendation, and “the model decided” language is refused.
