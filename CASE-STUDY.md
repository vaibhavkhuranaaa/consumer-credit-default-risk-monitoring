...
I built a read-only validation lab over 30,000 licensed academic credit records. It tests whether a selected non-demographic model is stable beyond simple references, shows uncertainty around five fixed review workloads, and lets a reviewer inspect retrospective score placement without turning that evidence into a lending decision.
...
Demographic fields—sex, education, marriage, and age—are excluded from training and public individual records. They remain local and are used only in an aggregate fairness audit. The public analyst artifact contains licensed non-demographic fields, derived measures, and retrospective out-of-fold scores.

## Evaluation design

### Keep the final audit final

The original split is fixed at 18,000 training rows, 6,000 validation rows, and a 6,000-row holdout. Model selection stays locked to train and validation evidence. The holdout identity is frozen by SHA-256 `df4a7f48dc14f491d592e858e1b128cb975ad83e1281de330876eb307cef2215` and remains outside subsequent selection.
...
Each approved workload—5%, 10%, 20%, 35%, and 50%—reports queue size, captured historical defaults, historical non-default reviews, precision, recall, lift, incremental yield, and bootstrap intervals.
...
1. establish portfolio and evidence status;
...
5. inspect a deterministic retrospective record placement;
...
A public record may show only its academic source ID, retrospective out-of-fold score, score band, deterministic rank/denominator, selected capacity, historical outcome, and `Inside simulated review set` or `Outside simulated review set`.
...
Before deployment, one gate regenerates and validates governed artifacts, runs the Python and web suites, builds production assets, and checks project records. GitHub Actions must pass for the same immutable candidate. The verified M10 release passed 19 Python tests, 15 web tests, TypeScript lint, the production build, artifact validators, and the repository gate.

The exact deployed application revision is `d3e5b103b63a8e222d36084c85eed302f6b35398`. Evaluation was generated at `2026-08-10T14:20:10.827594+00:00`; live verification completed at `2026-08-10T14:53:18Z`. The evaluation SHA-256 is `4d284b8f47fcf32ee599e1652a0ac0d09914784a5e5fd69f7fae8d6283c35fff`, and the deployed analyst artifact SHA-256 is `fb9ba15d059471904c8b982243aee6d36e69eb982b98a861b5b1b8478ec0247b`.
