...
def validate_existing_artifact(release_file: Path) -> str:
    if not release_file.is_file():
        raise ValueError(f"Release artifact not found: {release_file}")
    payload = read_json(release_file)
    validate_release(payload)
    artifact_hash = hashlib.sha256(release_file.read_bytes()).hexdigest()
    print(
        "Immutable aggregate artifact: pass "
        f"(release_id={payload['release_id']}, sha256={artifact_hash})"
    )
    return artifact_hash
...
def main() -> int:
    ...
    parser.add_argument(
        "--application-only",
        action="store_true",
        help="Validate the existing immutable release and application without regenerating model evidence.",
    )
    ...
    if args.application_only:
        validate_existing_artifact((ROOT / args.release_file).resolve())
    else:
