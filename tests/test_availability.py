...
def test_live_contract_rejects_protected_fields() -> None:
    ...
    with pytest.raises(ValueError, match="protected attributes"):
        validate_payloads(health, release, artifact, source_marker, "release-1", "abcdef1", "a" * 40)


def test_live_contract_rejects_wrong_deployed_source() -> None:
    health, release, artifact, source_marker = _payloads()
    source_marker["source_sha"] = "b" * 40
