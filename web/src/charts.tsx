...
export function ReviewFrontier({ data, selected, onSelect }: { data: ReviewScenario[]; selected: number; onSelect: (capacity: number) => void }) {
  const x = (capacity: number) => 74 + (capacity - .05) / .45 * 550; const y = (value: number) => 218 - value * 178; const queueMax = Math.max(...data.map((row) => row.queueSize), 1);
