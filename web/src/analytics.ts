...
export function activeFilters(filters: Filters): { label: string; reset: Partial<Filters> }[] {
  const active: { label: string; reset: Partial<Filters> }[] = [];
  if (filters.band !== "all") active.push({ label: `Score band: ${filters.band}`, reset: { band: "all" } });
  if (filters.outcome !== "all") active.push({ label: `Outcome: ${filters.outcome === "default" ? "Observed default" : "No observed default"}`, reset: { outcome: "all" } });
  if (filters.delinquency !== "all") active.push({ label: `Repayment: ${filters.delinquency}`, reset: { delinquency: "all" } });
  if (filters.limitMin !== DEFAULT_FILTERS.limitMin || filters.limitMax !== DEFAULT_FILTERS.limitMax) active.push({ label: `Limit: ${filters.limitMin.toLocaleString()}–${filters.limitMax.toLocaleString()}`, reset: { limitMin: DEFAULT_FILTERS.limitMin, limitMax: DEFAULT_FILTERS.limitMax } });
  if (filters.paymentRatioMin !== DEFAULT_FILTERS.paymentRatioMin || filters.paymentRatioMax !== DEFAULT_FILTERS.paymentRatioMax) active.push({ label: `Payment / bill: ${filters.paymentRatioMin.toFixed(2)}–${filters.paymentRatioMax.toFixed(2)}`, reset: { paymentRatioMin: DEFAULT_FILTERS.paymentRatioMin, paymentRatioMax: DEFAULT_FILTERS.paymentRatioMax } });
  if (filters.scoreMin !== DEFAULT_FILTERS.scoreMin || filters.scoreMax !== DEFAULT_FILTERS.scoreMax) active.push({ label: `Research score: ${(filters.scoreMin * 100).toFixed(0)}–${(filters.scoreMax * 100).toFixed(0)}%`, reset: { scoreMin: DEFAULT_FILTERS.scoreMin, scoreMax: DEFAULT_FILTERS.scoreMax } });
  return active;
}

export function describeFilters(filters: Filters): string[] {
  return activeFilters(filters).map(({ label }) => label);
}
...
export function reviewPlacement(record: CreditRecord, denominator: number, capacity: number) {
  ...
  return {
    cutoff,
    minimumCapacity: record.research_score_rank / denominator,
