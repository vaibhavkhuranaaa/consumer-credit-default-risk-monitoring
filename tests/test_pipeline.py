...
def test_private_manifest_path_supports_sibling_ops_and_explicit_override(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    repo = tmp_path / "credit-risk"
    monkeypatch.delenv("CREDIT_RISK_DATA_MANIFEST", raising=False)
    assert data_manifest_path(repo) == tmp_path / "credit-risk-ops" / ".project" / "data-manifest.yml"

    monkeypatch.setenv("CREDIT_RISK_DATA_MANIFEST", "config/private-data.yml")
    assert data_manifest_path(repo) == repo / "config/private-data.yml"
