...
it("returns a cacheable aggregate release without exposing database details or revisions", async () => {
  const query = vi.fn().mockResolvedValue([{ public_payload: { version: 1, scope: "aggregate only", code_revision: "private", source: { evaluated_revision: "private", command: "evaluate --revision aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" } } }]);
  ...
  await expect(response.json()).resolves.toEqual({ version: 1, scope: "aggregate only", source: { command: "evaluate --revision historical-release" } });
});
