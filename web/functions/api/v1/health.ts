...
type ReleaseHealth = { release_id: string; released_at: string | Date };
...
export const onRequestGet: PagesFunction<Bindings> = async ({ env, request }) => {
  ...
  try {
    ...
    const rows = await sql.query("SELECT release_id, released_at FROM public_release_snapshot LIMIT 1");
