...
const valid = {
  version: 4,
  source: { rows: 1, evaluation_sha256: "a", evaluation_schema_version: 2, evaluation_generated_at_utc: "2026-08-10T00:00:00Z", protected_attribute_boundary: "local fairness audit only" },
...
it("explains unreadable and unreachable artifact responses", async () => {
  const fetchMock = vi.fn()
    .mockResolvedValueOnce(new Response("<html>not data</html>", { status: 200 }))
    .mockRejectedValueOnce(new TypeError("network failed"));
  vi.stubGlobal("fetch", fetchMock);

  await expect(getPublicDataset()).rejects.toThrow("unreadable response");
  await expect(getPublicDataset()).rejects.toThrow("could not be reached");
});
