...
7. Build `web` with `pnpm build`, then deploy `web/dist` directly to the approved Cloudflare Pages project. Configure only `NEON_API_DATABASE_URL` and `ALLOWED_ORIGIN` as Cloudflare secrets. Pages Functions emit structured failure events to Cloudflare's existing logs; the Pages configuration deliberately omits the unsupported Workers-only `observability` block and creates no scheduled job or third-party monitor.
