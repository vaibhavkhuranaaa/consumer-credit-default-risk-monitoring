...
export function ChartFrame({ title, eyebrow, subtitle, meaning, decision, limitation, children, table, className = "" }: ChartFrameProps) {
  return <article className={`analysis-block ${className}`.trim()}>
    ...
    <div className="chart-region" role="region" aria-label={`${title} chart`} tabIndex={0}>{children}</div>
...
export function BandPopulationChart({ data, selected, onSelect }: { data: BandSummary[]; selected: string; onSelect: (band: string) => void }) {
  ...
  return <svg className="chart-svg" viewBox="0 0 680 280" role="group" aria-label="Population bars and observed default rate line by research score band">
...
export function DelinquencyChart({ data, selected, onSelect }: { data: { label: string; count: number; defaultRate: number }[]; selected: string; onSelect: (value: string) => void }) {
  ...
  return <svg className="chart-svg" viewBox="0 0 680 230" role="group" aria-label="Repayment status composition with observed default rate">
...
export function ReviewFrontier({ data, selected, onSelect }: { data: ReviewScenario[]; selected: number; onSelect: (capacity: number) => void }) {
  ...
  return <svg className="chart-svg" viewBox="0 0 680 280" role="group" aria-label="Review capacity frontier with workload bars, precision, and recall">
...
export function DistributionChart({ data, formatLabel, onSelect }: { data: { minimum: number; maximum: number; count: number; defaultRate: number }[]; formatLabel: (value: number) => string; onSelect?: (minimum: number, maximum: number) => void }) {
  ...
  return <svg className="chart-svg" viewBox="0 0 680 270" role={onSelect ? "group" : "img"} aria-label="Filtered cohort distribution with observed default rate">
...
export function SimpleTable({ headers, rows }: { headers: string[]; rows: (string | number)[][] }) {
  return <div className="table-scroll" tabIndex={0} role="group" aria-label={`${headers.join(", ")} evidence table`}><table className="data-table compact"><thead><tr>{headers.map((header)=><th key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((row,index)=><tr key={index}>{row.map((cell,cellIndex)=><td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table></div>;
}
