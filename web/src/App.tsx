...
const shortDate = new Intl.DateTimeFormat("en-US", { year: "numeric", month: "short", day: "numeric", timeZone: "UTC" });
const PAGE_SIZE = 20;
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <header className="masthead">
      ...
      <dl className="masthead-meta"><div><dt>Records</dt><dd>{number.format(records.length)}</dd></div><div><dt>Evidence date</dt><dd>{shortDate.format(new Date(dataset.evidence.generated_at_utc))}</dd></div></dl>
    </header>
    ...
    {view !== "review" && view !== "assurance" && <FilterBar filters={filters} setFilters={setFilters} capacity={capacity} setCapacity={setCapacity} showCapacity={view === "records"} resultCount={filtered.length} totalCount={records.length} />}
...
function FilterBar({ filters, setFilters, capacity, setCapacity, showCapacity, resultCount, totalCount }: { filters: Filters; setFilters: (filters: Filters) => void; capacity: number; setCapacity: (value: number) => void; showCapacity: boolean; resultCount: number; totalCount: number }) {
  const active = activeFilters(filters);
  const [draft, setDraft] = useState(filters);
  useEffect(() => setDraft(filters), [filters]);
  const update = <K extends keyof Filters>(key: K, value: Filters[K]) => setFilters({ ...filters, [key]: value });
  const updateDraft = (key: keyof Filters, value: number) =>
    setDraft((current) => ({ ...current, [key]: value }));
  const rangeError = validateRangeFilters(draft);
  const rangeDirty = JSON.stringify(draft) !== JSON.stringify(filters);
  return <section className="filter-shell" aria-label="Global cohort filters">
    <div className={`filter-primary${showCapacity ? "" : " cohort-only"}`}>
      ...
      <label>Research score band<select value={filters.band} onChange={(event) => update("band", event.target.value)}><option value="all">All bands</option>{SCORE_BANDS.map((band) => <option key={band}>{band}</option>)}</select></label>
      ...
      {showCapacity && <label>Review capacity<select value={capacity} onChange={(event) => setCapacity(Number(event.target.value))}>{[.05,.1,.2,.35,.5].map((value) => <option value={value} key={value}>{percent.format(value)}</option>)}</select></label>}
    ...
    <details className="advanced-filters">
      ...
      <form onSubmit={(event) => { event.preventDefault(); if (!rangeError) setFilters(draft); }}>
        <div className="range-grid">
          <fieldset><legend>Reported credit limit</legend><label>Minimum<input type="number" min="10000" max="1000000" step="10000" value={draft.limitMin} aria-invalid={!!rangeError} aria-describedby="range-feedback" onChange={(event) => updateDraft("limitMin", Number(event.target.value))}/></label><label>Maximum<input type="number" min="10000" max="1000000" step="10000" value={draft.limitMax} aria-invalid={!!rangeError} aria-describedby="range-feedback" onChange={(event) => updateDraft("limitMax", Number(event.target.value))}/></label></fieldset>
          <fieldset><legend>Payment-to-bill ratio</legend><label>Minimum<input type="number" min="0" max="10" step="0.05" value={draft.paymentRatioMin} aria-invalid={!!rangeError} aria-describedby="range-feedback" onChange={(event) => updateDraft("paymentRatioMin", Number(event.target.value))}/></label><label>Maximum<input type="number" min="0" max="10" step="0.05" value={draft.paymentRatioMax} aria-invalid={!!rangeError} aria-describedby="range-feedback" onChange={(event) => updateDraft("paymentRatioMax", Number(event.target.value))}/></label></fieldset>
          <fieldset><legend>Research score</legend><label>Minimum<input type="number" min="0" max="1" step="0.05" value={draft.scoreMin} aria-invalid={!!rangeError} aria-describedby="range-feedback" onChange={(event) => updateDraft("scoreMin", Number(event.target.value))}/></label><label>Maximum<input type="number" min="0" max="1" step="0.05" value={draft.scoreMax} aria-invalid={!!rangeError} aria-describedby="range-feedback" onChange={(event) => updateDraft("scoreMax", Number(event.target.value))}/></label></fieldset>
        </div>
        <div className="range-actions"><p id="range-feedback" className={rangeError ? "range-error" : "quiet"} role={rangeError ? "alert" : undefined}>{rangeError || "Range edits apply together after validation."}</p><div><button type="button" disabled={!rangeDirty} onClick={() => setDraft(filters)}>Discard edits</button><button type="submit" disabled={!rangeDirty || !!rangeError}>Apply ranges</button></div></div>
      </form>
    </details>
    <div className="filter-tokens">{active.length ? active.map(({ label, reset }) => <button key={label} onClick={() => setFilters({ ...filters, ...reset })} aria-label={`Remove ${label} filter`}><span>{label}</span><span aria-hidden="true">×</span></button>) : <span className="quiet">No cohort filters applied</span>}</div>
    <p className="sr-only" role="status">{number.format(resultCount)} of {number.format(totalCount)} records match the active cohort filters.</p>
  </section>;
}

function validateRangeFilters(filters: Filters) {
  const ranges = [
    ["Reported credit limit", filters.limitMin, filters.limitMax, 10_000, 1_000_000],
    ["Payment-to-bill ratio", filters.paymentRatioMin, filters.paymentRatioMax, 0, 10],
    ["Research score", filters.scoreMin, filters.scoreMax, 0, 1],
  ] as const;
  for (const [label, minimum, maximum, lowerBound, upperBound] of ranges) {
    if (!Number.isFinite(minimum) || !Number.isFinite(maximum) || minimum < lowerBound || maximum > upperBound) return `${label} must stay between ${lowerBound.toLocaleString()} and ${upperBound.toLocaleString()}.`;
    if (minimum > maximum) return `${label} minimum cannot exceed its maximum.`;
  }
  return "";
}
...
function AssuranceView({ dataset, release, health, supportUnavailable }: { dataset: PublicDataset; release: Release | null; health: Health | null; supportUnavailable: boolean }) {
  ...
  const serviceReady=health?.status==="ready"&&!supportUnavailable;
  const tiedChallenger=comparisons.some((row)=>row.overall_status==="tie");
  const limitedCohort=robustness.some((row)=>row.warning||row.sample_size_status!=="adequate");
  const gates=[
    {name:"Accountability",status:"Missing",evidence:"Decision owner, independent reviewer, approval date, and next review are not recorded in public evidence.",condition:"Record accountable governance before institutional use."},
    {name:"Temporal validation",status:"Missing",evidence:evidence.split.limitation,condition:"Add out-of-time or prospective evidence before any operational consideration."},
    {name:"Service verification",status:serviceReady?"Pass":"Unverified",evidence:serviceReady?"Read-only evidence service and current release reported ready.":"Support health is not verified in this session.",condition:serviceReady?"Reverify at each release.":"Verify health and release lineage before relying on the service."},
    {name:"Discrimination",status:tiedChallenger?"Conditional":"Pass",evidence:tiedChallenger?"Selected model remains tied with a credible challenger under the practical-margin rule.":"Recorded paired comparisons support the selected model.",condition:tiedChallenger?"Retain the tie; do not claim unique superiority.":"Retain paired monitoring against named baselines."},
    {name:"Calibration",status:selected.calibration_diagnostics.warning?"Conditional":"Pass",evidence:selected.calibration_diagnostics.warning??"No sparse calibration bin triggered the governed warning threshold.",condition:"Treat calibration as descriptive until verified out of time."},
    {name:"Cohort robustness",status:limitedCohort?"Conditional":"Pass",evidence:limitedCohort?"At least one non-demographic cohort is limited or carries an interpretation warning.":"Recorded non-demographic cohorts meet the sample-size rule.",condition:"Preserve sample-size warnings and avoid causal interpretation."},
    {name:"Development stability",status:Object.keys(development.models).length?"Pass":"Missing",evidence:development.limitation,condition:"Keep the frozen holdout outside model selection."},
    {name:"Lineage",status:"Pass",evidence:`Evaluation schema v${evidence.schema_version}; generated ${shortDate.format(new Date(evidence.generated_at_utc))}.`,condition:"Bind every release to the governed evaluation and source artifacts."},
    {name:"Public data boundary",status:"Pass",evidence:"Protected attributes are excluded from public individual records.",condition:"Keep aggregate fairness evidence local and separate."},
  ];
  const openGates=gates.filter((gate)=>gate.status!=="Pass"); const passedGates=gates.filter((gate)=>gate.status==="Pass");
  return <>
    <ViewIntro title="Model validation" description="Disposition, open conditions, paired baselines, calibration, stability, robustness, reliance, and lineage." source="Frozen holdout + repeated development evaluation" />
    <section className="disposition-board" aria-labelledby="disposition-title">
      <header><div><p className="section-label">Validation disposition</p><h2 id="disposition-title">Restricted to retrospective research</h2><p>{evidence.readiness.verdict} This public artifact contains no institutional approval.</p></div><span className="disposition-badge">Institutional use blocked</span></header>
      <div className="disposition-meta"><dl><div><dt>Lending use</dt><dd>Not approved</dd></div><div><dt>Decision owner</dt><dd>Not recorded</dd></div><div><dt>Independent reviewer</dt><dd>Not recorded</dd></div><div><dt>Next review</dt><dd>Not scheduled</dd></div><div><dt>Evidence generated</dt><dd>{shortDate.format(new Date(evidence.generated_at_utc))}</dd></div><div><dt>Open gates</dt><dd>{openGates.length} of {gates.length}</dd></div></dl><button type="button" onClick={()=>window.print()}>Print validation brief</button></div>
      <div className="gate-register" aria-label="Open validation conditions">{openGates.map((gate)=><article key={gate.name}><span className={`gate-status status-${gate.status.toLowerCase()}`}>{gate.status}</span><div><h3>{gate.name}</h3><p>{gate.evidence}</p><small>{gate.condition}</small></div></article>)}</div>
      <details className="passed-gates"><summary>Review {passedGates.length} passed evidence gates</summary><div>{passedGates.map((gate)=><article key={gate.name}><span className="gate-status status-pass">Pass</span><div><h3>{gate.name}</h3><p>{gate.evidence}</p><small>{gate.condition}</small></div></article>)}</div></details>
    </section>
    <section className="assurance-summary assurance-metrics" aria-label="Fixed holdout performance evidence">
    ...
    <section className="validation-grid" aria-label="Supporting validation evidence">
...
function RecordView({ records, filterLabels, capacity, denominator }: { records: CreditRecord[]; filterLabels: string[]; capacity:number; denominator:number }) {
  const [query,setQuery]=useState(""); const [sortKey,setSortKey]=useState<keyof CreditRecord>("ID"); const [direction,setDirection]=useState<"asc"|"desc">("asc"); const [page,setPage]=useState(0); const [selected,setSelected]=useState<CreditRecord|null>(null); const [selectionNotice,setSelectionNotice]=useState("");
  ...
  useEffect(()=>{if(selected&&!records.some((record)=>record.ID===selected.ID)){setSelected(null);setSelectionNotice("Inspector cleared because the selected record is outside the active cohort.");}},[records,selected]);
  ...
  const inspect=(record:CreditRecord)=>{setSelected(record);setSelectionNotice("");};
  return <>
    <ViewIntro title="Record simulation" description="Retrospective score, band, deterministic rank, capacity sensitivity, and historical outcome only." source={`Out-of-fold artifact rank · denominator ${number.format(denominator)}`} />
    {selectionNotice&&<p className="record-notice" role="status">{selectionNotice}</p>}
    <section className="record-layout">
      <div className="record-table-panel">
        <header><div><p className="section-label">Filtered workbench</p><h2>{number.format(searched.length)} research {searched.length===1?"record":"records"}</h2><p>{filterLabels.length?filterLabels.join(" · "):"All governed records"}</p></div><label>Search source ID <span>(partial accepted)</span><input type="search" inputMode="numeric" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="e.g. 18421"/></label></header>
        {visible.length?<><div className="table-scroll desktop-record-table"><table className="data-table records-table simulation-table"><thead><tr><Sortable label="Source ID" column="ID" current={sortKey} direction={direction} onSort={sort}/><Sortable label="Research score" column="research_score" current={sortKey} direction={direction} onSort={sort}/><Sortable label="Band" column="score_band" current={sortKey} direction={direction} onSort={sort}/><Sortable label="Rank" column="research_score_rank" current={sortKey} direction={direction} onSort={sort}/><Sortable label="Observed outcome" column="default payment next month" current={sortKey} direction={direction} onSort={sort}/><th>Minimum simulated capacity</th><th><span className="sr-only">Inspect</span></th></tr></thead><tbody>{visible.map((record)=>{const placement=reviewPlacement(record,denominator,capacity);return <tr key={record.ID} className={selected?.ID===record.ID?"selected-row":""}><td className="mono">#{record.ID}</td><td className="mono score-cell">{percent.format(record.research_score)}</td><td><span className={`band-label band-${record.score_band.toLowerCase().replace(" ","-")}`}>{record.score_band}</span></td><td className="mono">{number.format(record.research_score_rank)} / {number.format(denominator)}</td><td>{record["default payment next month"]?"Observed default":"No observed default"}</td><td><strong className="capacity-threshold">{percent.format(placement.minimumCapacity)}</strong></td><td><button className="text-button" onClick={()=>inspect(record)} aria-label={`Inspect research simulation for source record ${record.ID}`}>Inspect</button></td></tr>})}</tbody></table></div><div className="mobile-record-list">{visible.map((record)=>{const placement=reviewPlacement(record,denominator,capacity);return <button key={record.ID} className={selected?.ID===record.ID?"selected":""} onClick={()=>inspect(record)} aria-label={`Inspect research simulation for source record ${record.ID}`}><span className="mobile-record-head"><b>#{record.ID}</b><strong className="capacity-threshold">Minimum {percent.format(placement.minimumCapacity)}</strong></span><span className="mobile-record-score"><b>{percent.format(record.research_score)}</b><small>{record.score_band} research band</small></span><span className="mobile-record-meta"><span>Rank {number.format(record.research_score_rank)} / {number.format(denominator)}</span><span>Selected scenario {percent.format(capacity)}</span><span>{record["default payment next month"]?"Observed default":"No observed default"}</span></span></button>})}</div><div className="pagination"><label>Page<input aria-label="Page number" type="number" min="1" max={pages} value={current+1} onChange={(event)=>setPage(Math.min(pages,Math.max(1,Number(event.target.value)||1))-1)}/><span>of {pages}</span></label><div><button disabled={current===0} onClick={()=>setPage(current-1)}>Previous</button><button disabled={current+1>=pages} onClick={()=>setPage(current+1)}>Next</button></div></div></>:records.length===0?<div className="inline-empty"><strong>No records in the active cohort</strong><p>Remove or reset cohort filters above. No record evidence was inferred.</p></div>:<div className="inline-empty"><strong>No source ID matches this search</strong><p>Try a full or partial academic source ID.</p><button className="text-button" onClick={()=>setQuery("")}>Clear source ID search</button></div>}
      </div>
...
function RecordInspector({ record, capacity, denominator, onClose }: { record: CreditRecord|null; capacity:number; denominator:number; onClose:()=>void }) {
  if(!record)return <aside className="record-inspector empty"><p className="section-label">Simulation inspector</p><h2>Select one governed record</h2><p>The inspector shows only the retrospective out-of-fold score, score band, deterministic rank, capacity sensitivity, and historical outcome.</p><p>It refuses lending-decision, eligibility, pricing, adverse-action, recommendation, and “the model decided” language.</p></aside>;
  const placement=reviewPlacement(record,denominator,capacity);
  return <aside className="record-inspector" aria-live="polite"><header><div><p className="section-label">Academic source record</p><h2>#{record.ID}</h2></div><button aria-label="Close record inspector" onClick={onClose}>×</button></header><div className="record-signal"><span>{record.score_band} research band</span><strong>{percent.format(record.research_score)}</strong><p>{record["default payment next month"]?"Observed historical default":"No observed historical default"}</p></div><section className="simulation-placement"><p className="section-label">Capacity sensitivity</p><strong>Minimum {percent.format(placement.minimumCapacity)}</strong><p>At the selected {percent.format(capacity)} scenario, the simulated cutoff is rank {number.format(placement.cutoff)}. This record’s rank {number.format(record.research_score_rank)} falls {placement.inside?"within":"beyond"} that cutoff.</p><dl><div><dt>Deterministic rank</dt><dd>{number.format(record.research_score_rank)} / {number.format(denominator)}</dd></div><div><dt>Historical outcome</dt><dd>{record["default payment next month"]?"Observed default":"No observed default"}</dd></div></dl></section><div className="inspector-boundary"><strong>Retrospective research simulation only</strong><p>This rank sensitivity is not an approval, denial, price, adverse-action reason, or lending recommendation.</p></div></aside>;
}
...
function EvidencePanel({eyebrow,title,description,children}:{eyebrow:string;title:string;description:string;children:React.ReactNode}){return <article className="evidence-panel"><header><div><h2>{title}</h2><span>{eyebrow}</span></div><p>{description}</p></header><details><summary>Review supporting evidence</summary><div>{children}</div></details></article>}
...
function LoadingState(){return <div className="loading-shell" role="status" aria-live="polite"><div className="loading-mast"><i/><i/><i/></div><div className="loading-grid"><i/><i/><i/><i/><i/></div><div className="loading-charts"><i/><i/></div><p>Downloading and validating 30,000 governed records. This can take a few seconds on a slower connection.</p></div>}
