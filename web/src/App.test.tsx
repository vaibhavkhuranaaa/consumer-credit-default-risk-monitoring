...
const dataset = {
  ...
  evidence: {
    ...
    lineage: { source_sha256: "c".repeat(64), command: "test command", tool_versions: { python: "3.12" } },
...
it("loads the decision hierarchy and cross-filters to an empty governed cohort", async () => {
  ...
  const scoreRange = screen.getByRole("group", { name: "Research score" });
  fireEvent.change(within(scoreRange).getByLabelText("Maximum"), { target: { value: "0.05" } });
  fireEvent.change(within(scoreRange).getByLabelText("Minimum"), { target: { value: "0.1" } });
  expect(screen.getByRole("alert")).toHaveTextContent(/Research score/);
  expect(screen.getByRole("button", { name: "Apply ranges" })).toBeDisabled();
  expect(screen.getByText("2 of 2 records match the active cohort filters.")).toBeInTheDocument();
  fireEvent.click(screen.getByRole("button", { name: "Discard edits" }));
  fireEvent.change(screen.getByLabelText("Research score band"), { target: { value: "Elevated" } });
  expect(await screen.findByText("No records match this cohort")).toBeInTheDocument();
  expect(screen.getByText("0 of 2 records match the active cohort filters.")).toBeInTheDocument();
  fireEvent.click(screen.getByRole("button", { name: "Reset cohort filters" }));
  expect(await screen.findByText("Portfolio overview")).toBeInTheDocument();
  fireEvent.change(screen.getByLabelText("Research score band"), { target: { value: "High" } });
  fireEvent.click(screen.getByRole("button", { name: "Remove Score band: High filter" }));
  expect(screen.getByText("2 of 2 records match the active cohort filters.")).toBeInTheDocument();
});
...
it("preserves review, governance refusal, and constrained record simulation workflows", async () => {
  ...
  expect(screen.getByText("Restricted to retrospective research")).toBeInTheDocument();
  expect(screen.getByText("Institutional use blocked")).toBeInTheDocument();
  ...
  expect(screen.getByText("Minimum 50%", { selector: ".simulation-placement strong" })).toBeInTheDocument();
  expect(screen.getByText(/This rank sensitivity is not an approval/)).toBeInTheDocument();
  fireEvent.change(screen.getByLabelText("Research score band"), { target: { value: "Low" } });
  expect(screen.getByText("Inspector cleared because the selected record is outside the active cohort.")).toBeInTheDocument();
  expect(screen.getByText("Select one governed record")).toBeInTheDocument();
});
