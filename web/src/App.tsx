...
const NAVIGATION: { value: View; label: string; prompt: string }[] = [
  ...
  { value: "assurance", label: "Model validation", prompt: "Is added complexity supported?" },
  { value: "records", label: "Record simulation", prompt: "Where does a record rank?" },
];
  ...
  return <div className="app-shell">
    ...
    <header className="masthead">
      <div className="masthead-brand">
        ...
        <div><strong>Credit Risk Model Validation &amp; Review-Capacity Lab</strong><span>Retrospective UCI research benchmark</span></div>
      </div>
...
function AssuranceView({ dataset, release, health, supportUnavailable }: { dataset: PublicDataset; release: Release | null; health: Health | null; supportUnavailable: boolean }) {
  ...
  return <>
    <ViewIntro index="04 / Model validation" title="Does added model complexity earn research use?" description="Start with the readiness verdict, then inspect paired baselines, split stability, calibration, cohort robustness, model reliance, and missing validation." source="Frozen holdout + repeated development evidence" />
    ...
    <section className="validation-grid" aria-label="Repeated development validation evidence">
      <EvidencePanel eyebrow="Paired baseline verdicts" title="Is the selected model stably better than each reference?" description={`${development.method} · ${number.format(development.development_n)} development rows · identical folds across models`}>
      ...
      <EvidencePanel eyebrow="Split stability" title="How much does performance move across shared folds?" description="Repeated development evidence; not the frozen holdout">
      ...
      <EvidencePanel eyebrow="Non-demographic robustness" title="Where does selected-model evidence weaken?" description="Frozen holdout cohorts · sample sizes and intervals shown">
      ...
      <EvidencePanel eyebrow="Feature-group ablations" title="Which feature groups does model performance rely on?" description="Repeated development evidence · groups removed one at a time">
...
function RecordView({ records, filterLabels, capacity, denominator }: { records: CreditRecord[]; filterLabels: string[]; capacity:number; denominator:number }) {
  ...
  return <>
    <ViewIntro index="05 / Record simulation" title="Where does a record fall in the selected research review set?" description="Search the governed artifact and inspect only retrospective score, band, deterministic rank, capacity, historical outcome, and simulated placement." source={`Out-of-fold artifact rank · denominator ${number.format(denominator)}`} />
...
function EvidencePanel({eyebrow,title,description,children}:{eyebrow:string;title:string;description:string;children:React.ReactNode}){return <article className="evidence-panel"><header><p className="section-label">{eyebrow}</p><h2>{title}</h2><p>{description}</p></header>{children}</article>}
