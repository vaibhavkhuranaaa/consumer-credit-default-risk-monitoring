...
def enrich(frame, scores):
    ...
    enriched["research_score"] = scores.round(6)
...
def main():
    ...
    artifact={"version":PUBLIC_ARTIFACT_VERSION,"source":{"dataset_id":manifest["id"],"citation":manifest["citation"],"license":manifest["license"],"archive_sha256":manifest["archive_sha256"],"evaluation_sha256":hashlib.sha256(evaluation_path.read_bytes()).hexdigest(),"rows":len(frame),"columns":[column for column in frame.columns if column not in PROTECTED_COLUMNS],"selected_model":selected,"protected_attribute_boundary":"local fairness audit only"},"records":json.loads(enriched.to_json(orient="records")),"evidence":evaluation}
